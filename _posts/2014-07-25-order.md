...
An object with an operation name as the key and an array of the [Order Custom Field](#order_custom_field) data as the value for all operations except the `remove` operation. The `remove` operation only requires a list of custom field codes.<br />
...
<tbody>
    ...
    <tr>
        <th>select</th>
        <td>
            Select input type data must be an object with `label` or `id` key or both.<br />
    ...
    <tr>
        <th>multiselect</th>
        <td>
            Multi-select input type data must be an array of objects with `label` or `id` key or both.<br />
