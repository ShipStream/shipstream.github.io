...
Order Custom Fields data must be an object with the custom field code as the key and an object as the value for all operations except the `remove` operation. The `remove` operation only requires a list of custom field codes.<br />
