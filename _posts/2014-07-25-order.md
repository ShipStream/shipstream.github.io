...
custom_fields
: Object with Custom Fields. See "<a href="/ref/order-custom-field.html">Order Custom Field</a>".
{:.code-defs}
...
3 _object_
: Order Custom Fields (see "[Order Custom Fields](#order_custom_fields)")

<h3 id="order_custom_fields">
    Order Custom Fields
</h3>

: The key is an operation name. The value is an array of Order Custom Fields data. Keys can be `set`, `add`, `remove`, `add_option`, `remove_option`.<br />  
...
Order Custom Fields data must be an object with `code` and `value` keys for all operations except `remove` operation. `remove` operation only requires `code` key.<br />
`value` field for select input type must be an object with `label` or `id` key or both.<br />
`value` field for multi-select input type must be an array of objects with `label` or `id` key or both.<br />
{:.code-defs}
...
<tbody>
    ...
    <tr>
        ...
        <td>
            ...
            <div><table class="table-striped"><tbody>
                <tr><th>UI Label</th><th>Code</th></tr>
                <tr><td>Backordered<td><code>backordered</code></td></tr>
                <tr><td>Canceled<td><code>canceled</code></td></tr>
                <tr><td>Complete<td><code>complete</code></td></tr>
                <tr><td>Scheduled Hold<td><code>delayed_shipment</code></td></tr>
                <tr><td>On Hold<td><code>holded</code></td></tr>
                <tr><td>Invalid Address<td><code>invalid_address</code></td></tr>
                <tr><td>New<td><code>new</code></td></tr>
                <tr><td>Partial Backorder<td><code>partial_backorder</code></td></tr>
                <tr><td>Processing<td><code>processing</code></td></tr>
                <tr><td>Unable To Process<td><code>unable_to_process</code></td></tr>
                <tr><td>Unresolved Shipping Method<td><code>unresolved</code></td></tr>
              </tbody></table></div>
      </td>
    </tr>
    <tr>
